...
#    train_indices, test_indices = sampleFixNum.samplingFixedNum(TRAIN_NUM, gt)
    ...
    all_assign = indexToAssignment(range(ALL_SIZE), whole_pca32.shape[0], whole_pca32.shape[1], PATCH_LENGTH)
    for i in range(len(all_assign)):
        all_data[i] = selectNeighboringPatch(padded_pca32, all_assign[i][0], all_assign[i][1], PATCH_LENGTH)
